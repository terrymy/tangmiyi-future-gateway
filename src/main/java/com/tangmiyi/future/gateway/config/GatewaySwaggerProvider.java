...
public class GatewaySwaggerProvider implements SwaggerResourcesProvider {
    ...
    public List<SwaggerResource> get() {
        ...
        //取出gateway的route
        List<GatewayRouteDefinition> gatewayRouteDefinitions = gatewayRouteListSingle.getGatewayRouteDefinitionList();
        gatewayRouteDefinitions.forEach(route -> routes.add(route.getId()));
        //结合配置的route-路径(Path)，和route过滤，只获取有效的route节点
        gatewayRouteDefinitions.stream().filter(routeDefinition -> routes.contains(routeDefinition.getId()))
